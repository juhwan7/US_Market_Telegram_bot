...
def analyze_with_gemini(data, news):
    ...
    [작성 핵심 규칙]
    1. 강조할 문장이나 핵심 요약은 <b>텍스트</b> 태그로 굵게 처리해.
    2. 중요한 종목명, 섹터, 혹은 핵심 키워드는 반드시 <code>텍스트</code> 태그로 감싸서 하이라이트(음영) 처리를 해줘.
    3. 어려운 전문 용어는 직관적이고 쉬운 일상어로 바꿔서 설명하거나, 옆에 괄호로 뜻을 달아줘.
    4. 엔터(줄바꿈)를 넉넉히 쓰고, 시각적으로 깔끔하게 기호(■, ▶, 💡, 🎯 등)를 적절히 배치해.
    ...
    <b>■ 2. 돈이 몰린 섹터와 특징주</b>
    ▶ <b>강세 섹터</b>: (왜 올랐는지 쉬운 말로 설명. 관련 종목은 <code>종목명</code> 처리)
    ▶ <b>약세 섹터</b>: (왜 내렸는지 설명)
    💡 <b>국장 연결 고리</b>: (미장 흐름이 우리나라 특정 테마에 어떤 영향을 줄지 분석)

    <b>■ 3. 오늘 국장 대응 전략</b>
    🎯 <b>관심 집중 <code>테마</code></b>: (오늘 시초가부터 돈이 몰릴 섹터와 이유)
